...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun) // shotgun 
        {
            float shotgunBulletSpreadStart = shotgunSpread / 2;
            float shotgunBulletSpreadStep = shotgunSpread / shotgunBullets;

            for (int i = 0; i < shotgunBullets; i++)
            {
                SpawnBullet(
                  gameObject.transform.position,
                  Quaternion.Euler(0, 0, gameObject.transform.eulerAngles.z - shotgunBulletSpreadStart + (shotgunBulletSpreadStep * i))
              );
            }
        }
        else // normal
