...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        GameObject player = GameManager.Instance.NearestPlayer(gameObject.transform.position);
        if (player == null)
            return;

        
        gameObject.transform.rotation = Quaternion.LookRotation(player.transform.position - gameObject.transform.position, Vector3.forward);
        mRigidBody.MovePosition(Vector3.MoveTowards(gameObject.transform.position, player.transform.position, speed * Time.deltaTime));
